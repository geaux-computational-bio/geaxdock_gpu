...
__global__ void MonteCarlo_Init_d(const int rep_begin, const int rep_end) {
...
#if IS_RANDOMIZE_LIG == 1
      ForceMove_d(bidx, mylig, 200.5f);
#endif
...
__global__ void MonteCarlo_d(const int rep_begin, const int rep_end,

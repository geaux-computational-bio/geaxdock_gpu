...
__device__ void CalcEnergy_d(const int bidx, Ligand *__restrict__ mylig,
                             const Protein *__restrict__ myprt) {
  ...
  for (int i = 0; i < lna_dc; i += blockDim.y) {
    ...
    if (l < lna_dc) {
      ...
      for (int j = 0; j < pnp_dc; j += blockDim.x) {
        ...
        if (p < pnp_dc) {
          ...
          if (CUDA_LDG_D(myprt->c0_and_d12_or_c2[p]) == 1 &&
              dst_pow2 <= 81.0f) {
            a_val[threadIdx.y][threadIdx.x] +=
                CUDA_LDG_D(myprt->hpp[p]) *
                (1.0f -
                 (3.5f / 81.0f * dst_pow2 - 4.5f / 81.0f / 81.0f * dst_pow4 +
          ...
          /* L-J potential */
          const float p1 =
              enepara_p1a[lig_t][prt_t] / (dst_pow4 * dst_pow4 * dst);
          const float p2 = enepara_p2a[lig_t][prt_t] / (dst_pow4 * dst_pow2);
          const float p4 =
              p1 * enepara_lj0_dc * (1.0f + enepara_lj1_dc * dst_pow2) + 1.0f;
          evdw[bidx] += (p1 - p2) / p4;
#endif
          ...
          float g1;
          if (s1 < 1)
            g1 = enepara_el0_dc + enepara_a1_dc * s1 * s1 +
                 enepara_b1_dc * s1 * s1 * s1;
          else
            g1 = 1.0f / s1;
          eele[bidx] +=
              CUDA_LDG_D(mylig->c[l]) * CUDA_LDG_D(myprt->ele[p]) * g1;
#endif
          ...
          epmf[bidx] += enepara_pmf1[lig_t][prt_t] /
    ...
    if (threadIdx.x == 0 && l < lna_dc) {
      ...
      const float hpc2 =
          (a_val[threadIdx.y][0] - enepara_hpl0[lig_t]) / enepara_hpl1[lig_t];
  ...
  for (int i = 0; i < lna_dc; i += blockDim.y) {
      ...
      for (int j = 0; j < pnk_dc; j += blockDim.x) {
        ...
        if (k < pnk_dc) {
          ...
          if (CUDA_LDG_D(mylig->t[l]) == kde_dc->t[k]) {
            ...
            a_val[threadIdx.y][threadIdx.x] +=
                expf(enepara_kde2_dc * kde_dst_pow2);
    ...
    if (threadIdx.x == 0 && l < lna_dc && a_sz[threadIdx.y][0] != 0)
      ekde[threadIdx.y] +=
          (a_val[threadIdx.y][0] / (float) a_sz[threadIdx.y][0]);
  ...
  for (int i = 0; i < pos_dc; i += blockDim.y) {
    ...
    if (threadIdx.x == 0 && m < pos_dc && a_sz[threadIdx.y][0] != 0) {
      elhm[threadIdx.y] += mcs_dc[m].tcc * sqrtf(a_val[threadIdx.y][0] / (float)
                                                 a_sz[threadIdx.y][0]);
    }
  } // lhm loop

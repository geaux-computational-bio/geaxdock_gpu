__device__ void Move_d(const int bidx, Ligand *__restrict__ mylig,
                       const float p) {
  ...
  if (bidx < 6) {
    movematrix_new[bidx] =
        move_scale_dc[bidx] * p + mylig->movematrix_old[bidx];
...
__device__ void ForceMove_d(const int bidx, Ligand *__restrict__ mylig,
                            const float p) {
  __shared__ float movematrix_new[6]; // translation x y z, rotation x y z
  __shared__ float rot[3][3];         // rotz roty rotx

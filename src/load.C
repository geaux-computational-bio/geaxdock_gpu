...
void pushLigandPoint(Ligand0 *lig, int n, string a, int t, float c) {
  lig->n[n] = n;
  lig->a[n] = a;
  lig->t[n] = t;
  lig->c[n] = c;
 ...
 * the ligand effective total conf is less than the raw, since some small rmsd
 * excluded */
void loadLigConf(LigandFile *lig_file) {
  // ifstream
...
vector<float> getLigEnsembleRmsd(vector<string> sect) {
  int lnum = 0;
    ...
    if (iter_sdf->find("ENSEMBLE_RMSD") != string::npos) {
      while (true) {
        ...
        if (!isdigit(rmsd_line[0])) {
          ...
          for (auto rmsd_str = rmsd_strs.begin(); rmsd_str != rmsd_strs.end();
               ++rmsd_str) {
...
vector<string> getLigEnsembleCoords(vector<string> sect) {
...
void loadLigand(InputFiles *inputfiles, Ligand0 *lig) {
  try {
    ...
    std::cerr
        << "Unhandled Exception in loading one ligand section: " << e.what()
        << std::endl << "GeauxDock will now exit" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  try {
    ...
    std::cerr << "Unhandled Exception in trim ligand: " << e.what() << std::endl
...
int loadOneLigand(vector<string> sect, Ligand0 *ligs) {
...
void moveLigand2PocketCenter(float *pocket_center, Ligand0 *lig) {
  int lna = lig->lna;
...
void moveLigand2ItsCenterFrame(Ligand0 *lig) {
  int lna = lig->lna;
  float sum_x = 0.;
  float sum_y = 0.;
  float sum_z = 0.;
...
int getLigEnsembleTotal(vector<string> sect) {
  int ens_tot = -1;
  int lnum = 0;
  vector<string>::iterator iter_sdf;
  for (iter_sdf = sect.begin(); iter_sdf != sect.end(); iter_sdf++) {
...
void writeDefaultLigAtomCoord(vector<string> sect, Ligand0 *mylig) {
  string lines[MAXSDF];
  int lnum = 0;
...
void writeLigAtomProperty(vector<string> sect, Ligand0 *mylig) {
  string lines[MAXSDF];
  int lnum = 0;
  ...
  for (int i1 = 4; i1 < mylig->lna + 4; i1++) {
    pushLigandPoint(mylig, i1 - 4, lines[i1].substr(31, 24).c_str(),
                    tmp3[i1 - 4], tmp2[i1 - 4]);
  }
}

void writeLigProperty(vector<string> sect, Ligand0 *mylig) {
  string id_key = "MOLID";
  string lines[MAXSDF];
  int lnum = 0;
  int total_lines = sect.size();
...
vector<vector<string> > readLigandSections(string sdf_path) {
...
void loadLigand_bk(LigandFile *lig_file, Ligand0 *lig) {
  // ifstream
  std::string llib3 = lig_file->molid;
  list<string> l1_sdf;
  list<string>::iterator i1_sdf;
  ...
  for (int i1 = 4; i1 < mylig->lna + 4; i1++) {
    ...
    pushLigandPoint(mylig, i1 - 4, llib1[i1].substr(31, 24).c_str(),
                    tmp3[i1 - 4], tmp2[i1 - 4]);
  ...
  for (int i6 = 0; i6 < tmp7; i6++) {
    ...
    // u3b_(&weights, &mob_xyz, &ref_xyz, &(mylig->lna), &mode, &rms1, &u, &t,
    // &ier);
    ...
    if (rms2 > 0.1) {
      Ligand0 *my_rest_lig = &lig[*conf_ip]; // my_rest_lig points to other
                                             // conformations in the data file
...
void pushProteinPoint(Protein0 *prt, int r, int n, int t, int d, int c) {
  int i = prt->pnp;
...
void loadPrtConf(ProteinFile *prt_file, Protein0 *prt) {
  ...
  Protein0 *myprt = &prt[
      prt_conf]; // address of the protein with conformation number prt_conf
  myprt->pnp = 0;
  myprt->pnr = 0;
  ...
  for (p1_i = p1_data.begin(); p1_i != p1_data.end(); p1_i++) {
    if ((*p1_i).size() > 53) {
      ...
      if ((*p1_i).substr(0, 6) == "ATOM  ") {
        ...
        int residue1 =
            getResCode((*p1_i).substr(17, 3));     // 4th column, residue name
        int residue2 = atoi((*p1_i).substr(22, 4)
                                .c_str()); // 5th column, residue serial number
...
void loadProtein(ProteinFile *prt_file, Protein0 *prt) {
  ...
  for (int p2_i = 0; p2_i < (myprt->pnp); p2_i++) // iterating effective points
    ...
    for (p1_i = p1_data.begin(); p1_i != p1_data.end(); p1_i++) {
      ...
      //      cout << "1-- prt_conf: " << prt_conf << endl;
      //              myprt = &prt[prt_conf]; // address of the protein with
      // conformation number prt_conf
      ...
      if ((*p1_i).size() > 53) {
        if ((*p1_i).substr(0, 6) == "ATOM  ") {
          ...
          //cout << "tz2 = " << "ASCII " << (*p1_i).substr (46, 8).c_str () <<
          //"float " << tz2 << endl;
          ...
          if (residue4 == residue3) {
            if (point1 == 0 && atom2 == " CA ") {
              ...
              tn1 += 1.0;
            } else if (point1 == 2 || point1 == 3 || point1 == 4 || point1 ==
                       11 || point1 == 14 || point1 == 17 || point1 == 18 ||
                       point1 == 23 || point1 == 24 || point1 == 25) {
              if (atom2 != " N  " && atom2 != " CA " && atom2 != " C  " &&
                  atom2 != " O  ") {
                tx1 += tx2;
                ty1 += ty2;
                tz1 += tz2;
            ...
            } else if (point1 == 5 || point1 == 7 || point1 == 9 ||
                       point1 == 12 || point1 == 15 || point1 == 19 ||
            ...
            } else if (point1 == 6 || point1 == 8 || point1 == 10 ||
                       point1 == 13 || point1 == 16 || point1 == 20 ||
  ...
  for (int i = 1; i < num_prt_conf; i++) {
    ...
    Protein0 *myprt_rest =
        &prt[i]; // address of the protein with conformation number prt_conf
    myprt_rest->pnp = 0;
    myprt_rest->pnr = 0;
    pnp = 0;
    pnr = 0;
...
void pushKDEpoint(int at, float ax, float ay, float az, Kde0 *kde) {
  int i = kde->pnk;
  kde->n[i] = i;
  kde->t[i] = at;
  kde->x[i] = ax;
...
void loadPocketCenter(string lhm_path, float *center) {
  ...
  while (getline(h1_file, line1)) {
    if (line1.length() > 3) {
      ...
      if (line1.substr(0, 3) == "KDE") {
        ...
        pushKDEpoint(getLigCode(dat1[1]), atof(dat1[2].c_str()),
                     atof(dat1[3].c_str()), atof(dat1[4].c_str()), kde);
        // cout << "1-- pnk " << kde->pnk << endl;

        // cout << "2-- kde->pns[getLigCode (dat1[1])] " << kde->pns[getLigCode
        // (dat1[1])] << endl;
        kde->pns[getLigCode(dat1[1])]++;
        // cout << "3-- kde->pns[getLigCode (dat1[1])] " << kde->pns[getLigCode
        // (dat1[1])] << endl;
      } /* load PSP */
...
void loadEnePara(EneParaFile *enepara_file, EnePara0 *enepara) {
    ...
    if (line1.length() > 3) {
      if (line1.substr(0, 3) == "VDW") {
        ...
        enepara->vdw[getPntCode(dat1[1])][getLigCode(dat1[2])][1] =
            atof(dat1[4].c_str());
        /**< vdw[prtein pt][ligand pt][0] is the 1st parameter, and vdw[][][1]
         * is the 2nd */
  ...
  for (int ai = 0; ai < MAXTP4; ai++) {
    enepara->hpp[ai] = ((enepara->hpp[ai] - hpc1) / (hpc2 - hpc1)) * 2.0 - 1.0;
...
void loadNorPara(NorParaFile *norpara_file, EnePara0 *enepara) {
  ...
  for (iter = 0, data_i = data.begin();
       iter < total_item && data_i != data.end(); iter++, data_i++) {
  ...
  for (iter = 0, data2_i = data2.begin();
       iter < total_item && data2_i != data2.end(); iter++, data2_i++) {
...
void loadTrace(TraceFile *trace_file, float *trace) {
  ...
  for (trace_iter = 0, data_i = data.begin();
       trace_iter < total_trace_item && data_i != data.end();
       trace_iter++, data_i++) { // interate the list
    string s = (*data_i).substr(0, 30);
    istringstream os(s);
    float tmp = 0.0;
...
void loadWeight(WeightFile *weight_file, EnePara0 *enepara) {
  ...
  for (weight_iter = 0, data_i = data.begin();
       weight_iter < total_weight_item && data_i != data.end();
       weight_iter++, data_i++) { // interate the list
    string s = (*data_i).substr(0, 30);
    istringstream os(s);
    float tmp = 0.0;
    os >> tmp; // this tmp is what you need. do whatever you want with it
